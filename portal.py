...
if check_password():
    ...
    st.markdown("""
        <style>
        .header-box { background-color: #223a8e; padding: 3rem; border-radius: 20px; text-align: center; margin-bottom: 1rem; }
    ...
    if "user_email" in st.session_state:
        c_logout1, c_logout2, c_logout3 = st.columns([2,1,2])
    ...
    with tab3:
        ...
        if ad_pass == PASSWORD_ADMIN:
            ...
            n_em = col_a.text_input("Email del Cliente:")
            n_no = col_b.text_input("Nombre de Carpeta en Drive:")
            if st.button("REGISTRAR CLIENTE"):
                if n_em and n_no:
                    DICCIONARIO_CLIENTES[n_em.lower().strip()] = n_no
                    guardar_clientes(DICCIONARIO_CLIENTES)
                    st.success(f"Cliente {n_no} registrado.")
                    st.rerun()
            
            st.write("---")
            st.write("### 👥 Clientes con Acceso")
    ...
    if "user_email" not in st.session_state:
        ...
        with tab1:
            st.markdown(f'<div class="user-info">Cliente: {nombre_act}</div>', unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            ...
            try:
                ...
                if res_reg:
                    ...
                    if filas:
                        # Mostrar solo los últimos 5 envíos
                        for f in filas[-5:]:
                            st.text(f"📅 {f[0]} - 📄 {f[2]} (Ref: {f[3]})")
                    else: st.info("Aún no has enviado documentos.")
            except: pass

        with tab2:
            st.subheader("📥 Mis Impuestos")
            a_bus = st.selectbox("Año consulta:", ["2026", "2025"], key="bus_a")
            # ... (Lógica de impuestos igual que antes) ...
