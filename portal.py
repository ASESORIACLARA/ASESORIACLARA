...
if check_password():
    ...
    st.markdown("""
        ...
        .header-box h1 { color: white !important; margin: 0; font-size: clamp(1.3rem, 7vw, 2.2rem); font-weight: bold; }
        .user-info { background-color: #f0f4ff; padding: 8px; border-radius: 10px; color: #1e3a8a; font-weight: bold; margin-bottom: 10px; text-align: center; font-size: 0.85rem; }
        .status-panel { background: #ffffff; padding: 12px; border-radius: 12px; border: 1px solid #e2e8f0; text-align: center; margin-bottom: 15px; }
        ...
        .globo-aviso { border-radius: 12px; padding: 15px; margin: 10px 0; border-left: 5px solid; font-size: 0.9rem; }
    ...
    if "user_email" not in st.session_state:
        ...
        # SOLUCIÓN AL KEYERROR: Usar .get() con valores por defecto
        config_cliente = DATA_AVISOS.get(email_act, {
        ...
        st.markdown(f"""
            <div class="status-panel">
                ...
                <div class="badge {b_col}">{config_cliente.get('estado', 'Pendiente')}</div>
            </div>
        """, unsafe_allow_html=True)

        # Globos de Aviso con protección contra errores
        mensaje_texto = config_cliente.get('mensaje', "")
        if mensaje_texto:
            prio = config_cliente.get('prioridad', "Información")
            est_g = "aviso-urgente" if "Urgente" in prio else "aviso-finalizado" if "finalizado" in prio else "aviso-info"
            st.markdown(f"""
                <div class="globo-aviso {est_g}">
                    <div style="font-size: 0.7rem; opacity: 0.8; margin-bottom: 4px;">📅 {config_cliente.get('fecha', '')}</div>
                    {mensaje_texto}
                </div>
            """, unsafe_allow_html=True)
            ...
            if arc and st.button("🚀 SUBIR AHORA", use_container_width=True):
                try:
                    ...
                    if res:
                        ...
                        nuevo_nombre = f"{ahora.strftime('%Y-%m-%d')}_{tipo_sel}_REF-{ref}{os.path.splitext(arc.name)[1]}"
        ...
        with tab2:
            ...
            st.info("Aquí aparecerán tus modelos presentados del año " + a_bus)
        ...
        with tab3:
            ...
            if ad_pass == PASSWORD_ADMIN:
                ...
                if st.button("💾 ACTUALIZAR PORTAL", use_container_width=True):
                    ...
                    guardar_json(AVISOS_FILE, DATA_AVISOS)
                    st.success("Portal actualizado correctamente.")
                    st.rerun()
                
                st.markdown("---")
                st.write("### Gestión de Clientes")
                n_em, n_no = st.text_input("Nuevo Email:"), st.text_input("Nombre Drive:")
                if st.button("AÑADIR CLIENTE", use_container_width=True):
                    if n_em and n_no:
