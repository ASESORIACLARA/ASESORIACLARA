...
if check_password():
    ...
    if "u_mail" not in st.session_state:
        ...
        nom = D_CLI[mail]
        inf = D_AVI.get(mail, {"texto": "Bienvenido al portal.", "tipo": "informativo", "estado": "Pendiente documentación"})

        # --- LÓGICA DE COLORES DE ESTADO ---
        clase_estado = "estado-rojo"
        if inf["estado"] == "En revisión": clase_estado = "estado-amarillo"
        if inf["estado"] == "Presentado": clase_estado = "estado-verde"

        st.markdown(f'<div class="user-info">Sesión: {nom}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="{clase_estado}">📊 ESTADO 1T 2026: {inf["estado"].upper()}</div>', unsafe_allow_html=True)
        
        # --- LÓGICA DE AVISOS ---
        color_aviso = "#2196f3" if inf["tipo"] == "informativo" else "#f44336" if inf["tipo"] == "urgente" else "#ff9800"
        st.markdown(f'<div class="aviso-caja" style="border-color: {color_aviso}; background-color: {color_aviso}15;">📢 <b>AVISO:</b> {inf["texto"]}</div>', unsafe_allow_html=True)
            ...
            if arc and st.button("🚀 ENVIAR AHORA"):
                try:
                    ...
                    if res:
                        ...
                        # CREAR/ACTUALIZAR ARCHIVO DE TEXTO (REGISTRO)
        ...
        with tab2:
            ...
            a_b = st.selectbox("Selecciona el año para consultar:", ["2026", "2025"])
            ...
            if r_c:
                ...
                if r_a:
                    # Buscar subcarpeta de IMPUESTOS
