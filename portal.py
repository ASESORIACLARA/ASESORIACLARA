...
if check_password():
    ...
    if "u_mail" not in st.session_state:
        st.write("### 👋 Bienvenida")
        em = st.text_input("Introduce tu email:")
        ...
        nom = D_CLI[mail]
        inf = D_AVI.get(mail, {"texto": "Bienvenido al portal.", "estado": "Pte de documentación"})

        # Semáforo de Estado
        clase = "st-rojo"
        if "revisión" in inf["estado"].lower(): clase = "st-amarillo"
        elif "presentado" in inf["estado"].lower(): clase = "st-verde"

        st.markdown(f'<div class="user-info">Sesión: {nom}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="{clase}">📊 ESTADO: {inf["estado"].upper()}</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="aviso-caja">📢 <b>AVISO:</b> {inf["texto"]}</div>', unsafe_allow_html=True)
            ...
            if arc and st.button("🚀 ENVIAR AHORA"):
                try:
                    ...
                    if res:
                        ...
                        st.markdown(f'<div class="justificante">✅ Recibido correctamente. Referencia: {ref}</div>', unsafe_allow_html=True)
            ...
            if r_c:
                ...
                if r_a:
                    ...
                    if id_imp:
                        ...
                        if not docs: st.info("No hay documentos disponibles todavía.")
                        for d in docs:
                            buf = io.BytesIO()
                    ...
                    else: st.warning("No se encontró la carpeta 'IMPUESTOS PRESENTADOS'.")

        with t3:
            if st.text_input("Acceso Admin:", type="password") == PW_ADM:
                st.write("### Gestión de Clientes")
                c_sel = st.selectbox("Cliente:", list(D_CLI.keys()))
                new_av = st.text_area("Mensaje de Aviso:")
                new_est = st.selectbox("Estado:", ["Pte de documentación", "En revisión", "Presentado"])
                if st.button("ACTUALIZAR"):
                    D_AVI[c_sel] = {"texto": new_av, "estado": new_est}
                    save_j(AV_F, D_AVI); st.success("Portal actualizado.")
                
                st.write("---")
                for em, n in list(D_CLI.items()):
                    c_1, c_2 = st.columns([4,1])
                    c_1.write(f"👤 {n} ({em})")
                    if c_2.button("X", key=em):
                        del D_CLI[em]; save_j(DB_F, D_CLI); st.rerun()
                
                st.write("### Nuevo Cliente")
                ne, nn = st.text_input("Email:"), st.text_input("Nombre en Drive:")
